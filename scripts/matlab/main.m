...
for sheet_number = selected_participants
    ...
    for participant_dataset = participant_datasets
        ...
        clear all_features Flexion Extension Both

        % Generate Neural Features
        nfr_features = bhm_nfr.makeNeuralFeatures_NS5(nfr_spike_threshold, nip_time_kdf, nip_time_baseline, neural_data_ns5, neural_baseline_ns5);
        mav_features = zmh_mav.makeRollingPowerFeatures_zmh(nip_time_kdf, nip_time_baseline, neural_data_ns5, neural_baseline_ns5, mav_kernel_width);
        ...
        all_features = [nfr_features; mav_features; dwt_features];

        % Find used Kinematics
        valid_kinematic_idxs = [];
        for j = selected_finger_movements
            if sum(find(kinematics(j,:)))
                valid_kinematic_idxs = [valid_kinematic_idxs, j];
            end
        end

        % Separate training and testing sections for just combined movements
        [CombTrainMask, CombTestMask] = project_utils.separateTrials_bhm(nfr_features,kinematics(valid_kinematic_idxs,:),trial_struct,nip_time_kdf,1,1,0.75);
        [TrainMask, TestMask] = project_utils.separateTrials_bhm(nfr_features,kinematics(valid_kinematic_idxs,:),trial_struct,nip_time_kdf,0,0,0.75);
